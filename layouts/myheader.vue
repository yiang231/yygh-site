...
export default {
  ...
  mounted() {
    ...
    //当前大窗口的一个全局方法
    // loginCallback 方法名，methods中的loginCallback方法作为这个全局方法的方法体
    //在callback.vue中（小窗口）调用大窗口（全局）方法
    let self = this;
    window["loginCallback"] = (name, token, openid) => {
      self.loginCallback(name, token, openid);//methods中的普通的方法
    }
  ...
  methods: {
    ...
    loginCallback(name, token, openid) {
      if (openid !== '') {
        ...
        this.showLogin();//弹出框重新打开（默认显示输入手机号+验证码）== 绑定手机号
      } else {
        //不需要绑定手机号
    ...
    weixinLogin() {
      ...
      weixinApi.getLoginParam().then(resp => {
        var obj = new WxLogin({
          self_redirect: resp.data.self_redirect,
          id: resp.data.id,
