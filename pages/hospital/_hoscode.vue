  ...
  <div class="nav-container page-component">
    ...
    <div class="nav left-nav">
      <div class="nav-item selected">
                <span :onclick="'javascript:window.location=\'/hospital/'+hospital.hoscode+'\''"
                      class="v-link selected dark">预约挂号 </span>
      </div>
      <div class="nav-item ">
        <span :onclick="'javascript:window.location=\'/hospital/detail/'+hospital.hoscode+'\''"
              class="v-link clickable dark"> 医院详情 </span>
      </div>
      <div class="nav-item">
        <span :onclick="'javascript:window.location=\'/hospital/notice/'+hospital.hoscode+'\''"
    ...
    <div class="page-container">
      <div class="hospital-home">
        ...
        <div class="info-wrapper">
          <img :alt="hospital.hosname" :src="'data:image/jpeg;base64,'+hospital.logoData" class="hospital-img">
        ...
        <div class="select-dept-wrapper">
          ...
          <div class="sub-dept-container">
            <div v-for="(item,index) in departmentVoList" :id="item.depcode" :key="item.id"
                 :class="index === 0 ? 'selected' : ''" class="sub-dept-wrapper">
              <div class="sub-title">
...
export default {
  ...
  created() {
    //获取路由参数  http://localhost:3000/hospital/10000
  ...
  methods: {
    ...
    schedule(depcode) {
      ...
      if (!token) {
        //打开登录弹出框（myheader.vue）
        loginEvent.$emit('loginDialogEvent') //触发myheader.vue中的一个事件（事件名：loginDialogEvent）
        return
      }
      //2、查询当前用户的认证状态
      ...
      //3、显示当前科室的排班信息（authStatus==2 认证通过）
